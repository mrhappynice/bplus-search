...
if (sea.isSea()) {
  // Use a real filesystem-based require rooted at the executable
  const requireFromDisk = createRequire(process.execPath);
  // Assumes you copied node_modules/better-sqlite3/** next to the exe
  Database = requireFromDisk('better-sqlite3');
} else {
  // Normal dev/runtime
  try {
    Database = require('better-sqlite3');
  } catch {
    // Fallback if resolution is weird in your layout
    const requireFromCwd = createRequire(process.cwd() + path.sep);
    Database = requireFromCwd('better-sqlite3');
  }
}
...
// --- MODIFICATION START ---
// Determine the correct directory for persistent data like databases.
// In SEA, __dirname is '/virtual', which is not a real filesystem path.
// We must use the directory of the executable itself (process.execPath).
const storageDir = sea.isSea() ? path.dirname(process.execPath) : __dirname;
...
function extractSeaAssetsToTemp(prefix = 'public/') {
  ...
  if (_extractedDir) return _extractedDir;
...
// ---------------------------
// API: Conversation Management (Unchanged)
// ---------------------------
...
// ---------------------------
// API: Database Persistence (MODIFIED)
// ---------------------------
app.post('/api/research/save', async (req, res) => {
    ...
    // Use the reliable storageDir instead of __dirname
...
app.get('/api/research/files', (req, res) => {
    try {
        // Use the reliable storageDir instead of __dirname
        res.json(fs.readdirSync(storageDir).filter(file => file.endsWith('.db')));
    } catch (error) {
